...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      def loop(remaining: List[Future[T]], acc: List[T]): Unit = {
        remaining match {
          case Nil => p.success(acc.reverse)
          case head :: tail => 
            head.onComplete {
              case Failure(ex) => p.failure(ex)
              case Success(value) => loop(tail, value :: acc)
            }
        }
      }
      loop(fs, Nil)
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      fs.foreach {
        f => f.onComplete {
          result: Try[T] => p.tryComplete(result)
        }
      }
      p.future
    }
    ...
    def delay(t: Duration): Future[Unit] = {
      t match {
        case t if !t.isFinite => throw new IllegalArgumentException("Invalid duration")
        case t if t < Duration.Zero => throw new IllegalArgumentException("Negative duration")
        case _ => Future { 
          blocking {
