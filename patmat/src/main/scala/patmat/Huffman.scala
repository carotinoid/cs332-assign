...
object Huffman {
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    @tailrec
    def encoder(position: CodeTree, text: List[Char], acc: List[Bit]): List[Bit] = {
      text match {
        case Nil => acc
        case head :: tail => {
          position match {
            case Fork(left, right, _, _) => {
              if(chars(left).find(_ == head).isDefined) encoder(left, text, acc :+ 0)
              else encoder(right, text, acc :+ 1)
            }
            case Leaf(char, _) => encoder(tree, tail, acc)
          }
        }
      }
    }
    encoder(tree, text, Nil)
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def converter(branch: CodeTree, bitacc: List[Bit]): CodeTable = {
      branch match {
        case Leaf(char, _) => List((char, bitacc))
        case Fork(left, right, _, _) => 
          mergeCodeTables(converter(left, bitacc :+ 0), converter(right, bitacc :+ 1))
      }
    }
    converter(tree, Nil)
  }
